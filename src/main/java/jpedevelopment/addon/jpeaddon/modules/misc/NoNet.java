...
public class NoNet extends JPEModule {
    ...
    private final Setting<NoNet.ListMode> blocksFilter = sgSettings.add(new EnumSetting.Builder<NoNet.ListMode>()
        .name("blocks-filter")
        .description("How to use the block list setting")
        .defaultValue(NoNet.ListMode.Blacklist)
        .build()
    );
    ...
    private final Pool<NoNet.RenderBlock> renderBlockPool = new Pool<>(NoNet.RenderBlock::new);
    private final List<NoNet.RenderBlock> renderBlocks = new ArrayList<>();

    private final BlockPos.Mutable bp = new BlockPos.Mutable();
    private final BlockPos.Mutable prevBp = new BlockPos.Mutable();

    public NoNet() {
        super(JPEAddon.misc, "no-net", "Places a block under you at a set Y level, 100% chance to stop a void death.");
    }

    @Override
    public void onActivate() {
        for (NoNet.RenderBlock renderBlock : renderBlocks) renderBlockPool.free(renderBlock);
        renderBlocks.clear();
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        renderBlocks.forEach(NoNet.RenderBlock::tick);
    ...
    private boolean validItem(ItemStack itemStack, BlockPos pos) {
        ...
        if (blocksFilter.get() == NoNet.ListMode.Blacklist && blocks.get().contains(block)) return false;
        else if (blocksFilter.get() == NoNet.ListMode.Whitelist && !blocks.get().contains(block)) return false;
