...
import jpedevelopment.addon.jpeaddon.utils.ModuleH.*;
import jpedevelopment.addon.jpeaddon.utils.world.BlockHelper;
...
public class BedPlus extends JPEModule {
    ...
    public BedPlus() {
        super(JPEAddon.combat, "bed-plus", "Troll Module");
    }

    @Override
    public void onActivate() {
        target = null;
        ogItem = Wrap.getItemFromSlot(autoMoveSlot.get() - 1);
    ...
    public void onDeactivate() {
        if (safetyToggled) {
            warning("Tu Vida Esta Baja!!!");
            if (safetyGapSwap.get()) {
                FindItemResult gap = ItemH.findEgap();
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (mc.world.getDimension().bedWorks()) { error( "no tienes camas!"); toggle(); return; }
        ...
        if (preventEscape.get() && BlockHelper.getBlock(target.getBlockPos().up(2)) != Blocks.OBSIDIAN && Wrap.isInHole(target)) {
            FindItemResult obby = ItemH.findObby();
            if (obby.found()) {
                BlockUtils.place(target.getBlockPos().up(2), obby, true, 50, true, true, true);}
        ...
        if (breakSelfTrap.get() && shouldTrapMine()) {
            FindItemResult pick = ItemH.findPick();
            if (pick.found()) {
                Wrap.updateSlot(pick.slot());
                info("Breaking " + target.getEntityName() + "'s self-trap.");
                stb = BedU.getSelfTrapBlock(target, preventEscape.get());
                AutoH.doPacketMine(stb);
                sentTrapMine = true;
                return;
            }
        }
        if (placePos == null && AutoH.isBurrowed(target, false) && breakBurrow.get() && !sentBurrowMine) {
            FindItemResult pick = ItemH.findPick();
            if (pick.found()) {
                Wrap.updateSlot(pick.slot());
                info("Breaking " + target.getEntityName() + "'s burrow.");
                AutoH.doPacketMine(target.getBlockPos());
                sentBurrowMine = true;
                return;
            }
        }
        if (placePos == null && AutoH.isWebbed(target) && breakWeb.get()) {
            FindItemResult sword = ItemH.findSword();
            if (sword.found()) {
                Wrap.updateSlot(sword.slot());
                if (webTimer <= 0) {
                ...
                AutoH.mineWeb(target, sword.slot());
                return;
            }
        }

        if (sentTrapMine && didTrapMine()) { sentTrapMine = false; stb = null; }
        if (sentBurrowMine && !AutoH.isBurrowed(target, false)) sentBurrowMine = false;
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (renderAutomation.get() && target != null) {
            ...
            if (AutoH.isWeb(target.getBlockPos())) event.renderer.box(target.getBlockPos(), sideColor.get(), lineColor.get(), shapeMode.get(), 0);
            if (AutoH.isWeb(target.getBlockPos().up())) event.renderer.box(target.getBlockPos().up(), sideColor.get(), lineColor.get(), shapeMode.get(), 0);
        }
    }

    private boolean shouldTrapMine() {
        return !sentTrapMine && placePos == null && BedU.getSelfTrapBlock(target, preventEscape.get()) != null;
    }

    private boolean didTrapMine() {
        if (BedU.getSelfTrapBlock(target, preventEscape.get()) == null) return true;
        return BlockH.getBlock(stb) == Blocks.AIR || !AutoH.isTrapBlock(stb);
    }
