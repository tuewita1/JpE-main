...
import jpedevelopment.addon.jpeaddon.utils.ModuleH.BlockH;
import jpedevelopment.addon.jpeaddon.utils.ModuleH.ItemH;
...
public class AnchorPlus extends JPEModule {
    ...
    public AnchorPlus() {
        super(JPEAddon.combat, "anchor-plus", "kill yourself your enemy.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (antiStuck.get() && !sentAntiStuck) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && BlockH.getBlock(target.getBlockPos().up(2)) == Blocks.GLOWSTONE) {
                FindItemResult pick = ItemH.findPick();
                if (pick.found()) {
                    Wrap.updateSlot(pick.slot());
                    AutoH.doPacketMine(target.getBlockPos().up(2));
                    sentAntiStuck = true;
                    return;
                }
            }
        }
        if (sentAntiStuck && BlockH.getBlock(target.getBlockPos().up(2)) != Blocks.GLOWSTONE) sentAntiStuck = false;

        //Anti Self Trap
        if (breakSelfTrap.get() && !sentTrapMine) {
            if (findBreakPos(target.getBlockPos()) == null && findPlacePos(target.getBlockPos()) == null && AutoH.isTrapBlock(target.getBlockPos().up(2))) {
                FindItemResult pick = ItemH.findPick();
                if (pick.found()) {
                    Wrap.updateSlot(pick.slot());
        ...
        if (sentTrapMine && !AutoH.isTrapBlock(target.getBlockPos().up(2))) {
            if (fastPlaceAfter.get()) { placeDelayLeft = 0; breakDelayLeft = 0; }
            sentTrapMine = false;
        }

        //Anti Burrow
        if (breakBurrow.get() && !sentBurrowMine && AutoH.isBurrowed(target, true)) {
            FindItemResult pick = ItemH.findPick();
            if (pick.found()) {
                Wrap.updateSlot(pick.slot());
        ...
        if (sentBurrowMine && AutoH.isBurrowed(target, true)) {
            return;
        } else {
            sentBurrowMine = false;
        }
