...
public class SurroundV2 extends JPEModule {
    ...
    public SurroundV2() {
        super(JPEAddon.combat, "surround-plus", "Surround v2.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (BlockH.isVecComplete(getSurrDesign())) {
            if (disableAfter.get()) {
                info("Surround Completo.");
                toggle();
            }
        } else {
    ...
    private boolean isDangerousCrystal(BlockPos bp) {
        ...
        for (Vec3d b : getSurrDesign()) {
            BlockPos bb = ppos.add(b.x, b.y, b.z);
            if (!bp.equals(bb) && BlockH.distanceBetween(bb, bp) <= 2) return true;
        }
        return false;
    }
