...
public class SelfTrapV2 extends JPEModule {
    public enum modes {
        AntiFacePlace,
        Full,
        Top,
    ...
    public SelfTrapV2(){
        super(JPEAddon.combat, "self-trap-V2", "Places obsidian around your head.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (BlockH.isVecComplete(getTrapDesign()) && turnOff.get()) { info("Finished self trap."); toggle(); return;}
        if (toggleOnMove.get() && startPos != mc.player.getBlockPos()) { toggle(); return; }
        if (onlyInHole.get() && !Wrap.isInHole(mc.player)) { toggle(); return; }
        for (Vec3d b : getTrapDesign()) {
            if (bpt >= blockPerTick.get()) return;
            BlockPos ppos = mc.player.getBlockPos();
            BlockPos bb = ppos.add(b.x, b.y, b.z);
            if (BlockH.getBlock(bb) == Blocks.AIR) {
    ...
    private void onRender(Render3DEvent event) {
        if (!render.get() || BlockH.isVecComplete(getTrapDesign())) return;
        for (Vec3d b: getTrapDesign()) {
            BlockPos ppos = mc.player.getBlockPos();
            BlockPos bb = ppos.add(b.x, b.y, b.z);
            if (BlockH.getBlock(bb) == Blocks.AIR) event.renderer.box(bb, sideColor.get(), lineColor.get(), shapeMode.get(), 0);
        }
    }
